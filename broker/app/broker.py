...
def udp_receiver(s):
    ...
    while True:
        ...
        if addr not in devices.values(): # Caso o dado recebido venham de um dispositivo não cadastrado
            ...
            data[id] = msg
            print(f"Novo dispositivo cadastrado {addr}")
            print(f"{msg} recebidos de {addr} via UDP")
            id += 1
        else:
...
def tcp_listener(s):
    ...
    while True:
        ...
        if msg.decode() == 'init': # Caso a mensagem da primeira conexão TCP seja 'init'
            ...
            print(f"Conexão TCP feita com {addr}")
            print(conns)
            id += 1
...
def tcp_sender(s):
    while True: # Enquanto tiver comandos pendentes na lista commands esses comandos são enviados para o respectivo dispositivo
        if len(commands) > 0:
            ...
            print(f"Comando {commands[0]['command']} enviado via TCP para o Sensor {commands[0]['sensor']} {devices[commands[0]['sensor']]}")
            commands.pop(0)
...
def check_device_conn():
    while True:
        ...
        for i in conns.copy().keys():
            try:
                #print(f"Checando conexão com {conns[i]}")
                conns[i].send("test".encode())
                msg = conns[i].recv(1024)
                if not msg:
                    print(f"A conexão com o Sensor {i} foi perdida")
                    data.pop(i)
                    devices.pop(i)
                    conns.pop(i)
            except:
...
host = get_ip()
print(f"Broker operando no ip: {get_ip()}")
